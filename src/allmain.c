...
/* 
 * Modified on June 24, 2001 by Jaakko Peltonen:
 * kbhit() was mapped to some strange function on WinNT,
 * so I conditionally replace it with jtp_kbhit.
 */

#include "hack.h"

#ifdef USE_DIRECTX_SYSCALLS
#include "jtp_gen.h" /* For jtp_kbhit and jtp_getch */
#endif
...
#ifdef USE_DIRECTX_SYSCALLS
	    if (jtp_kbhit()) {
		if ((ch = jtp_getch()) == ABORT)
		    abort_lev++;
#else
	    if (kbhit()) {
		if ((ch = Getchar()) == ABORT)
		    abort_lev++;
#endif /* USE_DIRECTX_SYSCALLS */
# ifdef REDO
#ifndef USE_DIRECTX_SYSCALLS
		else
		    pushch(ch);
#endif /* USE_DIRECTX_SYSCALLS */
# endif /* REDO */
